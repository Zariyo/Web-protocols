...
import { addActorAction } from "../../ducks/actors/ActorActions";
...
const ActorForm = ({ history, addActorAction }, props) => {
    ...
    const handleSubmit = async (values) => {
        try {
            const actorToAdd = await axios.post("http://localhost:5000/actors", values)
            console.log("123")
            console.log(actorToAdd.data)
            addActorAction(actorToAdd.data);
        }
        catch (err) {
            console.log(err)
        }
