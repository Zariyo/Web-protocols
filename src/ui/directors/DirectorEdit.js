...
const DirectorForm = ({ history,director, editDirectorAction }, props) => {
    useEffect(() => {
    ...
    const handleSubmit = async (values) => {
        try {
            const directorToAdd = await axios.put(`http://localhost:5000/directors/${director._id}`, values)
            console.log("123")
            console.log(directorToAdd.data)
            editDirectorAction(directorToAdd.data);
        }
        catch (err) {
            console.log(err)
        }
    ...
    return (
        <div>
            <h3>Edycja reżysera</h3>
            <Formik
                initialValues={{
                    name: director.name,
                    surname: director.surname,
                    birthDate: new Date(director.birthDate).toISOString().slice(0, 10),
